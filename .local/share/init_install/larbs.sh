...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/vlagh3/funkydots.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/vlagh3/funkydots/main/.local/share/init_install/progs_all.csv"
  ...
  case "$(readlink -f /sbin/init)" in
    ...
    *)
      ...
      for repo in extra community; do
        grep -q "^\[$repo\]" /etc/pacman.conf || echo "[$repo] Include = /etc/pacman.d/mirrorlist-arch" >> /etc/pacman.conf
      done
...
geminstall() {
  dialog --title "LARBS Installation" --infobox "Installing the Ruby package \`$1\` ($n of $total). $1 $2" 5 70
  [ -x "$(command -v "gem")" ] || installpkg ruby >/dev/null 2>&1
  yes | gem install --no-user-install "$1"
}
...
systembeepoff() { 
  ...
  rmmod pcspkr
  echo "blacklist pcspkr" > /etc/modprobe.d/nobeep.config
}
